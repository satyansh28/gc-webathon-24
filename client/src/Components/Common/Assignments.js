...
const Assignments = () => {
    ...
    if(user.role==="student"){
      ...
      fetch(
        process.env.REACT_APP_BACKEND +
          "/api/academics/courses/"+window.location.href.split("/").pop()+"/assignments" 
          ,
        {
          credentials: "include",
        }
  ...
  const handleAssignmentView = (index) => {
    ...
    fetch(process.env.REACT_APP_BACKEND+"/api/academics/courses/"+window.location.href.split("/").pop()+"/assignments",{
      method:"POST",
      credentials:"include",
      headers: {
      ...
      body:JSON.stringify({...assign,points:undefined,dueDate:assign.dueDate.toISOString(),question:assign.question+"/n"+assign.url})
    })
		setEdit(false);
	};
